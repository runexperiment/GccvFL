...
def jugeResult(output,outFile):
    ...
    output = output.strip('\n')
    if not ACout==output:
        print(ACout,'end\n\n',output,'end\n\n\n',ACout == output,"   ",outFile,"  ",outFile,type(output),'\n______________________\n')
    if ACout ==  (output):
        return 1
    return 0
...
def getCoverInfo(programList,item,cata):
    ...
    for program in programList:    # program {"cata":os.path.join(pathOfSingleProblemData,itemDetial,itemAc),"name":itemAc}
        ...
        if Setting.cumulativeCoverTimes:  # 如果要累计覆盖次数那么就在这里编译一次即可
            ss = subprocess.Popen(languageType + " -fprofile-arcs -ftest-coverage " + compileFileName + " -o " +programName, shell=True,cwd=Setting.gcovDirName)
            ss.wait()
        # print(programName)
        ...
        for index , In in enumerate( item[Setting.IN]):
            ...
            if not Setting.cumulativeCoverTimes: # 不累加覆盖次数 ，对于每个测试用例，都需要重新编译一次
                ss = subprocess.Popen(languageType + " -fprofile-arcs -ftest-coverage " + compileFileName + "  -lm -o " +programName, shell=True,cwd=Setting.gcovDirName)
                ss.wait()
            # 将测试数据输入到程序中
            p = subprocess.Popen('./'+programName +" < " + common.jointPath(["../", Setting.TEST, In]), shell=True,cwd=Setting.gcovDirName,  stdout=subprocess.PIPE,stderr=subprocess.PIPE )
            p.wait()
            ...
            if Setting.saveCoverInfo:  # 如果选择保留文件，则将其文件名后面加个1，不保存的话，就会直接覆盖掉原来的文件
                ...
                subprocess.Popen("mv "+ program["name"] +".gcov  " + program["name"] +".gcov"+str(index+1), shell=True,cwd=Setting.gcovDirName).wait()
            extractCoverInfo.start(item['code'],gcovFileName,program["name"],result,In,program['errorLineList'])
            # sleep(2)
            # if Setting.dev["Main"]:
            #     break
        # if Setting.dev["Main"]:
...
if __name__ == "__main__":
    ...
    with open(Setting.problemInfoFileName, 'rb') as f:
        ...
        if Setting.getWaInfo:
            getCoverInfo(item[Setting.WA],item,common.jointPath([item[Setting.problemDirName],Setting.gcovDirName]))

        if Setting.getAcInfo:
            getCoverInfo(item[Setting.AC],item,common.jointPath([item[Setting.problemDirName],Setting.gcovDirName]))
