...
def DispartTestIO(pathOfSingleProblemData,path):
    if dev:
        print (path)
    problemIO = common.dirList(os.path.join(pathOfSingleProblemData, Setting.TEST))
    # print (problemIO)
    problemIn=[]
    problemOut=[]
...
def init(path=""):
    ...
    print("开始获取数据目录")
    problemPathList = common.dirList(path) # 一系列problem的文件名list
    ...
    for item in (problemPathList):
        ...
        problemDatail = common.dirList(pathOfSingleProblemData)
        # 获取输入输出文件
        """IO文件列表"""
        ...
        for itemDetial in (problemDatail):   # itemDetial = TESTDATA ,
            ...
            if Setting.AC in itemDetial:
                getAcList = common.dirList(os.path.join(pathOfSingleProblemData,itemDetial))
            ...
            if Setting.WA in itemDetial:
                getWaList = common.dirList(os.path.join(pathOfSingleProblemData, itemDetial))
